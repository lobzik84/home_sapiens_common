...
public class DBTools {
    ...
    public static DataSource getDataSource(String resourceName) throws Exception {
        javax.naming.InitialContext ctx = new javax.naming.InitialContext();
        javax.sql.DataSource ds = (javax.sql.DataSource) ctx.lookup(resourceName);
        return ds;
    }
    ...
    /**
     * Метод возвращает Map содержащий данные в виде экземпляра класса
     * ColumnData для каждого столбца таблицы, ключами являются имена столбцов
     *
     * @throws java.sql.SQLException
     * @return Map
     * @param conn - SQL соединение
    ...
    public static void deleteRow(String tableName, HashMap paramsMap, Connection conn) throws SQLException {
        ...
        if (PKFieldNameArr.length == 0) {
            throw new SQLException("No PK columns");
        }
    ...
    public static void updateRow(String tableName, HashMap paramsMap, Connection conn) throws SQLException {
        ...
        for (int i = 0; i < PKFieldNameArr.length; i++) {
            ...
            String columnName = PKFieldNameArr[i];
            if (setFieldsList.contains(columnName)) {
                continue;
            }
        ...
        for (int i = 0; i < columnNameArr.length; i++) {
            ...
            if (!paramsMap.containsKey(columnName)) {
                continue;
            }
            if (PKKeysTbl.containsKey(columnName)) {
                continue;
            }
    ...
    public static int insertRow(String tableName, HashMap paramsMap, Connection conn) throws SQLException {
        ...
        for (int i = 0; i < columnNameArr.length; i++) {
            ...
            if (paramsMap.get(columnName) == null) {
                continue;
            }
            ColumnData columnData = (ColumnData) columnDataMap.get(columnName);
            int columnType = columnData.SQLTypeId;
        ...
        if (paramsMap.containsKey(PKFieldName)) {
            paramsMap.remove(PKFieldName);
        }
    ...
    private static void setVals(PreparedStatement ps, ArrayList valArray, ArrayList typeArray) throws SQLException {
        if (valArray == null || typeArray == null || valArray.size() != typeArray.size()) {
            throw new SQLException("Invalid params or SQL types array");
        }
            ...
            switch (typeId) {
                ...
                case Types.LONGVARBINARY:
                    if (data instanceof byte[]) {
                        ps.setBytes(i + 1, (byte[]) data);
                    } else {
                        ps.setBytes(i + 1, data.toString().getBytes());
                    }
                    break;
	        ...
	        break;*/
                default:
                    ...
                    if (data instanceof Integer) {
                        ps.setInt(i + 1, ((Integer) data).intValue());
                    }
                    if (data instanceof Long) {
                        ps.setLong(i + 1, ((Long) data).longValue());
                    }
                    if (data instanceof Float) {
                        ps.setFloat(i + 1, ((Float) data).floatValue());
                    }
                    if (data instanceof Double) {
                        ps.setDouble(i + 1, ((Double) data).doubleValue());
                    }
            }
        }
    }
    ...
    public static java.util.List getOpenConnData() {
        ...
        for (Iterator itr = keySet.iterator(); itr.hasNext();) {
            dataArr.add(openConnectionsMap.get(itr.next()));
        }
        return dataArr;
    }
    ...
    private static void clearConnectionMap() {
        ...
        for (Iterator itr = removeArr.iterator(); itr.hasNext();) {
            openConnectionsMap.remove(itr.next());
        }
    }
